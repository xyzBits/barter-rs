# Barter-Execution
从金融交易场所流式获取私有账户数据，并执行（实盘或模拟）订单。此外还提供功能丰富的 MockExchange（模拟交易所）和 MockExecutionClient（模拟执行客户端），以协助回测和模拟交易。

**主要特点：**
* **简单易用**：ExecutionClient trait 提供了统一且简单的语言来与交易所交互。
* **标准化**：允许您的策略使用相同的接口与每个真实或模拟交易所通信。
* **可扩展**：Barter-Execution 高度可扩展，使添加新的交易所集成变得容易！

**请参阅：[`Barter`]、[`Barter-Data`]、[`Barter-Instrument`] 和 [`Barter-Integration`] 获取其他 Barter 库的完整文档。**
...
## 概述
高性能且标准化的交易接口，能够在多个金融交易场所执行交易。还提供功能丰富的模拟交易所，以协助回测和模拟交易。通过初始化交易所关联的 `ExecutionClient` 实例来与交易所通信。

## 示例
* 请参阅[此处][barter-examples]获取 Barter-Instrument 的使用示例。
